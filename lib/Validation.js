...
function checkparamSize(parameter,type){
    switch (type) {
        case "email":
...
async function TokenValidator(token) {
    ...
    let resault = await knex('users').select('accesstoken').where('accesstoken',token)
    resault = resault[0]
...
module.exports = {TokenGenerate, checkEmail,checkparamSize, TokenValidator}