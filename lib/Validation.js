...
async function accessToken(token = null){ 
    var x = await knex.select('accesstoken').from("users").where('accesstoken', token)
    if(x.length != 0){
        return 1
    }else {
        return 0
...
module.exports = {TokenGenerate, checkEmail,accessToken}